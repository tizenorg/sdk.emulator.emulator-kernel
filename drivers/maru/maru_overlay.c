						...
						const struct pci_device_id *ent)
{
	 ...
	 * because of the camera device is hold the /dev/video0.
	 */
	/* set to /dev/video1 */
	if (video_register_device(svo.video_dev0, VFL_TYPE_GRABBER, 1) < 0) {
	...
	/* set to /dev/video2 */
	if (video_register_device(svo.video_dev1, VFL_TYPE_GRABBER, 2) < 0) {
		printk(KERN_ERR "svo: video_register_device failed\n");
