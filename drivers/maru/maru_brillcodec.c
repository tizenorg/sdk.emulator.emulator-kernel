...
static void codec_bh(struct brillcodec_device *dev);
...
static void maru_brillcodec_info_cache(void)
{
...
static irqreturn_t maru_brillcodec_irq_handler(int irq, void *dev_id)
{
...
static void maru_brillcodec_context_remove(struct user_process_id *pid_elem)
{
	struct list_head *pos, *temp;
	struct context_id *cid_elem = NULL;
...
static void maru_brillcodec_task_add(uint32_t user_pid)
{
	struct user_process_id *pid_elem = NULL;
	unsigned long flags;
...
static void maru_brillcodec_task_remove(uint32_t user_pid)
{
	struct list_head *pos, *temp;
	struct user_process_id *pid_elem = NULL;
	unsigned long flags;
			...
			if (pid_elem) {
				if (pid_elem->id == user_pid) {
					...
					DEBUG("remove task. user_pid: %x, pid_elem: %p\n",
							user_pid, pid_elem);
					maru_brillcodec_context_remove(pid_elem);
				}
...
static int brillcodec_open(struct inode *inode, struct file *file)
{
	...
	if (request_irq(brillcodec_device->dev->irq, maru_brillcodec_irq_handler,
		IRQF_SHARED, DEVICE_NAME, brillcodec_device)) {
		ERROR("failed to register irq handle\n");
		return -EBUSY;
	}

	maru_brillcodec_task_add((uint32_t)file);
...
static int brillcodec_release(struct inode *inode, struct file *file)
{
	...
	/* free resource */
	maru_brillcodec_task_remove((uint32_t)file);
...
const struct file_operations maru_brillcodec_fops = {
	.owner			 = THIS_MODULE,
...
static struct miscdevice codec_dev = {
	...
	.name			= DEVICE_NAME,
	.fops			= &maru_brillcodec_fops,
	.mode			= S_IRUGO | S_IWUGO,
};

static bool maru_brillcodec_get_device_version(void)
{
...
static int maru_brillcodec_probe(struct pci_dev *pci_dev,
	const struct pci_device_id *pci_id)
{
	...
	if (!maru_brillcodec_get_device_version()) {
		return -EINVAL;
	}
...
static void maru_brillcodec_remove(struct pci_dev *pci_dev)
{
	if (brillcodec_device) {
		if (brillcodec_device->ioaddr) {
			iounmap(brillcodec_device->ioaddr);
...
static struct pci_device_id maru_brillcodec_pci_table[] = {
	{
...
MODULE_DEVICE_TABLE(pci, maru_brillcodec_pci_table);

static struct pci_driver driver = {
	.name		= DEVICE_NAME,
	.id_table	= maru_brillcodec_pci_table,
	.probe		= maru_brillcodec_probe,
	.remove		= maru_brillcodec_remove,
};

static int __init maru_brillcodec_init(void)
{
	printk(KERN_INFO "%s: driver is initialized.\n", DEVICE_NAME);

	codec_bh_workqueue = create_workqueue ("maru_brillcodec");
...
static void __exit maru_brillcodec_exit(void)
{
...
module_init(maru_brillcodec_init);
module_exit(maru_brillcodec_exit);
