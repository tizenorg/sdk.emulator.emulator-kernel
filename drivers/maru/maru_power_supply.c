...
static void get_power_data(int type)
{
	...
	err = virtqueue_add_outbuf(v_power->vq, v_power->sg_vq, 1, &v_power->msginfo, GFP_ATOMIC);
	if (err < 0) {
		DLOG(KERN_ERR, "failed to add out buffer to virtqueue (err = %d)", err);
		return;
	}
	err = virtqueue_add_inbuf(v_power->vq, v_power->sg_vq, 1, &v_power->msginfo, GFP_ATOMIC);
	if (err < 0) {
		DLOG(KERN_ERR, "failed to add in buffer to virtqueue (err = %d)", err);
		return;
	}
